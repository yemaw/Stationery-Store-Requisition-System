<%@ Page Title="" Language="C#" MasterPageFile="~/TemplatesWeb/DepartmentHead_Template.Master" AutoEventWireup="true" CodeBehind="HeadReport.aspx.cs" Inherits="PresentationLayer.DepartmentHead.HeadReport" %>
...
<%--<link rel='stylesheet' href='http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.18/themes/base/jquery-ui.css'/>--%>
    ...
    <script type="text/javascript">
        ...
        function retrieveBarChartData() {
            ...
            var quantityTypeName = document.getElementById("ContentPlaceHolder1_filterByDropDownList").value;
            var quantityType = 1;
            if (quantityTypeName == "Price") {
                quantityType = 2;
            }
            ...
            barCharJsonText = barCharJsonText + "," + quantityType;

            $.ajax(
            {
                type: "POST",
                // type: "GET",
                url: "HeadReport.aspx/GetAllDisbursements",
        ...
        function loadAllRecords(barChartData, monthyears) {
            ...
            $('#stationaryPriceTable tbody').html('');
            for (var i = 0; i < barChartData.labels.length; i++) {
                ...
                if (quantityTypeName == "Requested Quantity") {
                    $(tableRow).appendTo('#stationaryTable tbody');
                    document.getElementById("divStationeryPriceTable").style.visibility = "hidden";
                    document.getElementById("divStationeryTable").style.visibility = "visible";
                }
                else if (quantityTypeName == "Price") {
                    $(tableRow).appendTo('#stationaryPriceTable tbody');
                    document.getElementById("divStationeryPriceTable").style.visibility = "visible";
                    document.getElementById("divStationeryTable").style.visibility = "hidden";
                }
            }
        } //loadAllRecords
...
<div class="content_title_container">
        <asp:Label ID="ui_content_title_label" runat="server" Text="Requisitions Analysis Report" class="content_title"></asp:Label> 
    </div>
    ...
    <asp:TableRow>    
        ...
        <asp:TableCell RowSpan="3">
            ...
            <asp:Button ID="removeAllBtn" CssClass="btn btn-small" style="width:40px;" runat="server" Text="<<" Font-Bold="True" 
    ...
    <asp:Table ID="Table2" runat="server" Height="50px">
        <asp:TableRow>
            <asp:TableCell RowSpan="3">            
                <asp:DropDownList ID="filterByDropDownList" runat="server" OnChange="return retrieveBarChartData();">
                    <asp:ListItem Value="Requested Quantity" Selected="True" />   
                    <asp:ListItem Value="Price" Selected="False" />   
                </asp:DropDownList>
            </asp:TableCell>

             <asp:TableCell>
                <asp:Button ID="showBtn" CssClass="btn btn-medium btn-info" style="margin:0 10px 10px 10px;" runat="server" Text="Show Statistics" OnClientClick="return retrieveBarChartData();"/>   
             </asp:TableCell>  
             
             <asp:TableCell>
                <input type="button" class="btn btn-medium btn-info" style="margin:0 10px 10px 10px;" value="Print" onclick="Print()" />
             </asp:TableCell>  
        </asp:TableRow>        
    </asp:Table>
    </div>    
    ...
    <div class="yesPrint">
       <canvas id="canvas" width="680px" height="600px"></canvas>

        <br />
        <br />
       <div id="reportTable">
           <div id="divStationeryTable">
            ...
            <div id="divStationeryPriceTable">
                <table id='stationaryPriceTable'  style="width:100%" class="table table-hover table-striped" >
                    <thead><tr><td>Category</td><td>Date</td><td>Total Price</td></tr></thead>
                    <tbody>
                    </tbody>
                </table>
            </div>
